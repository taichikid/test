...
if __name__ == "__main__":
    ...
    try:
        ...
        start_time = time.time()
        start_degree = check_direction_pi(master)
        while time.time() - start_time < total_time:
            ...
            # 回転判定
            current_degree = check_direction_pi(master)
            if args[1] == "r":
                yaw = 1500 + power_yaw
                if current_degree < 0 & abs(current_degree) < 45:
                    yaw = 1500 + (power_yaw / 5)
                    if abs(current_degree) < 5:
                        yaw = 1500
            if args[1] == "l":
                yaw = 1500 - power_yaw
                if current_degree > 0 & abs(current_degree) < 45:
