...
if __name__ == "__main__":
    ...
    try:
        ...
        start_time = time.time()
        while time.time() - start_time < total_time + prepare_time:
            ...
            nda_theta = np_list_data[:,0]
            nda_distance = np_list_data[:,1]
            nda_x = np_list_data[:,2]
            nda_y = np_list_data[:,3]
            
            if np.count_nonzero(nda_distance < 0) > len(nda_distance) / 50:
                print("butukaruzo! hanten!")
                danger_degree = nda_theta[np.argmin(nda_distance)]
            ...
            dict_rc["pitch"] = pitch
            dict_rc["roll"] = roll
            if time.time() - start_time > prepare_time:
                dict_rc["roll"] = 1500
            print(dict_rc)
            th_send_msg_rc = set_rc(dict_rc, th_send_msg_rc)
    except:
